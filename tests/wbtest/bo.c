		      ...
		      uint32_t offsets[4], const char *pic_name)
{
	...
	/*
	 * Workaround for VBD Alpha window (Tentatively treat those using DRM_FORMAT_R8 format
	 * as VBD alpha window), which will read an extra burst16 (16x16 = 256 bytes) of data
	 */
	...
	/* Load pattern from file if specified */
	if (pic_name) {
...
void bo_destroy(struct bo *bo)
{
	...
	drmIoctl(bo->fd, DRM_IOCTL_MODE_DESTROY_DUMB, &arg);
	free(bo);
}
